...
public void OnEntityDestroyed(int entity)
{
	...
	if (usedBy != -1 && EntRefToEntIndex(PlayerData(usedBy).rSkinItem) == entity)
	{
		...
		if (!PlayerData(usedBy).inventoryChanged) PrintToChat(usedBy, "[FreakModels] Heads up, your skin has been removed.");
		else PlayerData(usedBy).inventoryChanged = false; //it was cahnged, set it to false
	}
}
